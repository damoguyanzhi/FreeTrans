package com.senocular.display {
	...
	public class TransformTool extends Sprite {
		...
		private var _moveNewTargets:Boolean = true;
		...
		public function set target(d:DisplayObject):void {
			...
			trace(_moveNewTargets,_moveEnabled,_moveControl);
		...
		public function set registration(p:Point):void {
			...
			trace("_registration:",_registration.x,_registration.y);
			innerRegistration = toolInvertedMatrix.transformPoint(_registration);
			trace("innerRegistration:",innerRegistration.x,innerRegistration.y);
		...
		/**缩放控制*/
		public var isShowScaleControls:Boolean=true;
		public function removeScaleControls():void
		{
			...
			removeToolControl(scaleControls, _scaleTopLeftControl);
			removeToolControl(scaleControls, _scaleTopRightControl);
			removeToolControl(scaleControls, _scaleBottomRightControl);
			removeToolControl(scaleControls, _scaleBottomLeftControl);
			
			isShowScaleControls=false;
		}
		
		public function addScaleControls():void
		{
			addToolControl(scaleControls, _scaleTopControl);
			addToolControl(scaleControls, _scaleRightControl);
			addToolControl(scaleControls, _scaleBottomControl);
			...
			isShowScaleControls=true;
		}
		/**注册控制*/
		public var isShowRegistrationControls:Boolean=true;
		public function removeRegistrationControls():void
		{
			removeToolControl(registrationControls, _registrationControl);
			isShowRegistrationControls=false;
		}
		
		public function addRegistrationControls():void
		{
			addToolControl(registrationControls, _registrationControl);
			isShowRegistrationControls=true;
		}
		/**旋转控制*/
		public var isShowRotateControls:Boolean=true;
		/**移除旋转功能*/
		public function removeRotateControls():void
		{
			removeToolControl(rotateControls, _rotationTopLeftControl);
			removeToolControl(rotateControls, _rotationTopRightControl);
			removeToolControl(rotateControls, _rotationBottomRightControl);
			removeToolControl(rotateControls, _rotationBottomLeftControl);
			
			isShowRotateControls=false;
		}
		
		public function addRotateControls():void
		{
			addToolControl(rotateControls, _rotationTopLeftControl);
			addToolControl(rotateControls, _rotationTopRightControl);
			addToolControl(rotateControls, _rotationBottomRightControl);
		...
		private function setNewRegistation():void {
			if (_rememberRegistration && _target in registrationLog) {
				...
				var pointX:Number=_target.width/2;
				var pointY:Number=_target.height/2;
				trace(pointX,pointY);
				innerRegistration = new Point(pointX,pointY);
...
class TransformToolInternalControl extends TransformToolControl {
	...
	public function position(event:Event = null):void {
		...
		//trace("change Position:",x,y);
	}
	
	private function controlMove(event:Event):void {
		//trace(interactionMethod);
		/*
		原：
		...
		2009年7月29日 by usun 
		site:http://weuio.com
		email:nbvghost@gmail.com
		QQ:274455411
		*/
		if (interactionMethod != null && _transformTool && _transformTool.currentControl == this) {
			interactionMethod();
		}
	}
}
